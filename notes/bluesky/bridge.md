...
Note that if you make an `ls` card and return to it later, after deleting a few files, those files will be gone. Waking a card restarts the process or returns to it, depending. If you want to freeze the text issued by an ls call, make the card with `ls | cat`. Or if you want an interactive session, we have something like `snap | ls`, which makes a snap of the directory at the present time and pipes that snapshot to an `ls` session. When you return to that card, you'll have a record of that directory at that time. This is clearly only going to work with a versioned filesystem or a directory under version control. 

What was this video business? I thought we were in the Matrix? We're not on a terminal, though we're using an evolution of the xterm protocol to drive the bridge. It's not challenging to request a frame of cells and render pixels onto it using graphics acceleration, requesting a fractional number of cells is what's impossible. Normally our bridge will want to render text into cells, but exceptions are normal among the Fleet.
